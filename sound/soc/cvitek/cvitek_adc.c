...
static u32 old_adc_voll;
static u32 old_adc_volr;
			       ...
			       struct snd_soc_dai *dai)
{
	...
	if (rate >= 8000 && rate <= 48000) {
		...
		dac1 = ioremap(0x28109000, 0x30);
		writel(ana0, dac1 + AUDIO_PHY_TXDAC_ANA0);
		iounmap(dac1);
		iounmap(dac);
	} else {
...
static int cv181xadc_reg_write(struct snd_soc_component *codec, unsigned int reg, unsigned int value)
{
	struct cv181xadc *adc = dev_get_drvdata(codec->dev);
	u32 temp_lval;
	u32 temp_rval;
...
static const struct snd_soc_component_driver soc_component_dev_cv181xadc0 = {
	.controls = cv181xadc_controls0,
	.num_controls = ARRAY_SIZE(cv181xadc_controls0),
	.read = cv181xadc_reg_read,
	.write = cv181xadc_reg_write,
};

static const struct snd_soc_component_driver soc_component_dev_cv181xadc1 = {
	.controls = cv181xadc_controls1,
	.num_controls = ARRAY_SIZE(cv181xadc_controls1),
	.read = cv181xadc_reg_read,
	.write = cv181xadc_reg_write,
};
...
static int adc_device_register(struct cv181xadc *adc)
{
	...
	sprintf(name, "misccvitekadc_%d", adc_comp_num);
...
static int cv181xadc_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "cvitekaadc get adc_base=0x%p\n", adc->adc_base);
	...
	dev_info(&pdev->dev, "mclk_source_addr=0x%x\n", mclk_source_addr);
	if (mclk_source_addr)
	...
	if (adc_comp_num)
		...
		comp_drv = &soc_component_dev_cv181xadc1;

	dev_info(&pdev->dev, "adc_comp_num:%d\n", adc_comp_num);
	adc_comp_num++;
	return devm_snd_soc_register_component(&pdev->dev, comp_drv,
					  &cv181xadc_dai, 1);
}
...
MODULE_ALIAS("platform:cvitekaadc0");
