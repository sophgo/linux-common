...
static struct snd_soc_dai_link_component cv181x_dac_cpus[] = {
	{
		.name = "29140000.i2s",
		.dai_name = "29140000.i2s",
	},
	{
		.name = "29160000.i2s",
		.dai_name = "29160000.i2s",
	},
...
static struct snd_soc_dai_link_component cv181x_dac_codecs[] = {
	{
		.name = "2810a000.dac",
		.dai_name = "2810a000.dac",
	},
	{
		.name = "28109000.dac",
		.dai_name = "28109000.dac",
	},
...
static struct snd_soc_dai_link cv181x_dac_dai[] = {
	{
		...
		.cpus = &cv181x_dac_cpus[0],
		.num_cpus = 1,
		.codecs = &cv181x_dac_codecs[0],
		.num_codecs = 1,
		.platforms = &cv181x_dac_platform[0],
		.num_platforms = 1,
		.ops = &cv181x_dac_ops,
	...
	{
		.name = "cviteka-i2s-dac1",
		.stream_name = "cviteka-dac1",
		.cpus = &cv181x_dac_cpus[1],
		.num_cpus = 1,
		.codecs = &cv181x_dac_codecs[1],
		.num_codecs = 1,
		.platforms = &cv181x_dac_platform[1],
		.num_platforms = 1,
		.ops = &cv181x_dac_ops,
		.init = cv181x_dac_codec_init,
		.dai_fmt = SND_SOC_DAIFMT_I2S
...
static struct snd_soc_card cv181x_dac = {
	...
	.num_links = ARRAY_SIZE(cv181x_dac_dai),
...
static int cv181x_dac_probe(struct platform_device *pdev)
{
	...
	if (np) {
		...
		card->dev = &pdev->dev;
		dev_info(&pdev->dev, "cviteka_adc_probe, card_name=%s\n", card->name);
		if (!proc_audio_dir) {
...
#ifdef CONFIG_PM
static int cv181x_cv181xdac_suspend(struct device *dev)
{
	return 0;
}

static int cv181x_cv181xdac_resume(struct device *dev)
{
	return 0;
}

static int cv181x_cv181xdac_poweroff(struct device *dev)
{
	return 0;
}
