...
static unsigned int cv181xdac_reg_read(struct snd_soc_component *codec, unsigned int reg)
{
...
static int cv181xdac_reg_write(struct snd_soc_component *codec, unsigned int reg, unsigned int value)
{
	struct cv181xdac *dac = dev_get_drvdata(codec->dev);
	u32 temp_lval;
	u32 temp_rval;
...
static const struct snd_soc_component_driver soc_component_dev_cv181xdac0 = {
	...
	.num_controls = ARRAY_SIZE(cv181xdac_controls0),
	.read = cv181xdac_reg_read,
	.write = cv181xdac_reg_write,
};

static const struct snd_soc_component_driver soc_component_dev_cv181xdac1 = {
	.controls = cv181xdac_controls1,
	.num_controls = ARRAY_SIZE(cv181xdac_controls1),
	.read = cv181xdac_reg_read,
	.write = cv181xdac_reg_write,
};
...
static int dac_device_register(struct cv181xdac *dac)
{
	...
	sprintf(name, "misccvitekdac_%d", dac_comp_num);
...
static int cv181xdac_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "cvitekadac_probe get dac_base=0x%p\n", dac->dac_base);
	...
	dac->mute_pin.mute_pin_l = of_get_named_gpio(pdev->dev.of_node,
                               "mute-gpio-l", 0);
	dac->mute_pin.mute_pin_r = of_get_named_gpio(pdev->dev.of_node,
                               "mute-gpio-r", 0);
	...
	dev_info(&pdev->dev, "dac_comp_num:%d\n", dac_comp_num);
	dac_comp_num++;
...
MODULE_ALIAS("platform:cvitekadac0");
