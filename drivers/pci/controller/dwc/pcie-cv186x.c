...
/* static int dw_pcie_wr_own_conf(struct dw_pcie_rp *pp, int where, int size,
			       u32 val)
{
	struct dw_pcie *pci;
...
static void cv186x_pcie_msi_init(struct dw_pcie_rp *pp)
{
...
static int cv186x_pcie_host_init(struct dw_pcie_rp *pp)
{
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
	struct cv186x_pcie *cv186x_pcie = to_cv186x_pcie(pci);
...
static struct dw_pcie_host_ops cv186x_pcie_host_ops = {
	.init = cv186x_pcie_host_init,
};

static int cv186x_pcie_add_dw_pcie_rp(struct cv186x_pcie *cv186x_pcie,
		struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_PCI_MSI)) {
		pp->msi_irq[0] = platform_get_irq_byname(pdev, "msi");
		if (pp->msi_irq[0] <= 0) {
...
static void cv186x_pcie_remove(struct platform_device *pdev)
{
...
static int cv186x_pcie_probe(struct platform_device *pdev)
{
	...
	ret = cv186x_pcie_add_dw_pcie_rp(cv186x_pcie, pdev);
	if (ret < 0)
		return ret;
