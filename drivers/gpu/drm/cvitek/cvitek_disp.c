									...
									struct drm_atomic_state *state)
{
	...
	if (!old_state) {
		DRM_ERROR("No old_state found for atomic enable\n");
		return;
	}
									...
									struct drm_atomic_state *state)
{
	...
	old_state = drm_atomic_get_old_crtc_state(state, crtc);
	if (!old_state) {
		DRM_ERROR("No old_state found for atomic disable\n");
		return;
	}
...
static void disp_crtc_atomic_flush(struct drm_crtc *crtc, struct drm_atomic_state *state)
{
	struct cvitek_crtc *ccrtc = to_cvitek_crtc(crtc);
	struct drm_crtc_state *old_state;
	...
	if (!old_state) {
		DRM_ERROR("No old_state found for atomic flush\n");
		return;
	}
								...
								struct drm_atomic_state *atomic_state)
{
	...
	if (plane->type == DRM_PLANE_TYPE_PRIMARY) {
		...
		if (src_w != crtc_w || src_h != crtc_h) {
			DRM_ERROR("Display not support scale, please use vpss\n");
			return -EINVAL;
		}
		...
		if (src_w != crtc_w || src_h != crtc_h) {
			DRM_ERROR("VGOP not support scale, please use vpss\n");
			return -EINVAL;
		}
	}
			       ...
			       u32 src_y, u32 src_w, u32 src_h)
{
	...
	if (type == DRM_PLANE_TYPE_PRIMARY) {
		...
		if (fmt == DISP_FORMAT_XRGB_8888) {
			...
			addr = dma_obj->dma_addr + fb->offsets[0];
			fmt = VGOP_FORMAT_ARGB8888;

#if defined(CONFIG_CVITEK_DRM_DEBUG)
			ctx->debugfs_disp_state[disp_id].dump_info.dma_object_vaddr[0] = (u64)(dma_obj->vaddr + fb->offsets[0]);
...
#if defined(CONFIG_CVITEK_DRM_DEBUG)
				ctx->debugfs_disp_state[disp_id].dump_info.dma_object_vaddr[i] =
					(u64)(dma_obj->vaddr + fb->offsets[i]);
...
#if defined(CONFIG_CVITEK_DRM_DEBUG)
		ctx->debugfs_disp_state[disp_id].dump_info.dma_object_vaddr[0] =
			(u64)(dma_obj->vaddr + fb->offsets[0]);
		ctx->debugfs_disp_state[disp_id].dump_info.pitches[0] = fb->pitches[0];
									...
									struct drm_atomic_state *atomic_state)
{
	...
	state = drm_atomic_get_new_plane_state(atomic_state, plane);
	// Sometimes it's not necessary to check 'state', since we only care about the plane type
	// if (!state) return;
...
static void cvitek_disp_remove(struct platform_device *pdev)
{
