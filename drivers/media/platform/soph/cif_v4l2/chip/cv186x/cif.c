...
{
#if (KERNEL_VERSION(6, 0, 0) <= LINUX_VERSION_CODE)
	struct cvi_cif_dev *dev = (struct cvi_cif_dev *)pde_data(file_inode(file));
#else
...
{
#if (KERNEL_VERSION(6, 0, 0) <= LINUX_VERSION_CODE)
	struct cvi_cif_dev *dev = (struct cvi_cif_dev *)pde_data(inode);
#else
