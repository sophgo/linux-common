			      ...
			      struct v4l2_mbus_config *mbus)
{
	...
	/* Set input_mode according to mbus->type */
...
struct cvi_cif_async_subdev {
	struct v4l2_async_connection asd;// rename v4l2_async_subdev to v4l2_async_connection
	struct v4l2_mbus_config mbus;
};
...
static int cif_parse_of(struct cvi_cif_dev *dev)
{
	...
	struct cvi_cif_async_subdev *cif_async = &dev->cif_async;
	int ret;

	ep = fwnode_graph_get_endpoint_by_id(dev_fwnode(dev->dev), 0, 0, 0);
	if (!ep) {
		CIF_PR(CIF_DEBUG, "No fwnode endpoint found!\n");
		return 0;
	}
	...
	if (ret) {
		CIF_PR(CIF_ERROR, "Failed to parse %pOF\n", to_of_node(fwnode));
		ret = -EINVAL;
		goto out;
	}
	...
	if (vep.bus_type != V4L2_MBUS_BT656 &&
	    ...
	    vep.bus_type != V4L2_MBUS_CSI2_CPHY &&
	    vep.bus_type != V4L2_MBUS_CCP2) {
		CIF_PR(CIF_DEBUG, "Unsupported bus type: %d\n", vep.bus_type);
		ret = 0;
		goto out;
	}

	cif_async->mbus.type = vep.bus_type;

	asc = v4l2_async_nf_add_fwnode(&dev->notifier, fwnode,
				       struct v4l2_async_connection);
	if (IS_ERR(asc)) {
		ret = PTR_ERR(asc);
		CIF_PR(CIF_ERROR, "Failed to add fwnode to async notifier: %d\n", ret);
		goto out;
	}

	dev->cif_async.asc = asc;
out:
