...
static struct os04a10_mode supported_modes[] = {
	{
		...
		.sns_type = V4L2_OV_OS04A10_MIPI_4M_1440P_30FPS_12BIT,
		.sns_type_name  = "V4L2_OV_OS04A10_MIPI_4M_1440P_30FPS_12BIT",
		.max_fps = {
			.numerator = 10000,
			.denominator = 300000,
		},
	...
	},
	{
		...
		.sns_type = V4L2_OV_OS04A10_MIPI_4M_1440P_30FPS_10BIT_WDR2TO1,
		.sns_type_name  = "V4L2_OV_OS04A10_MIPI_4M_1440P_30FPS_10BIT_WDR2TO1",
		.max_fps = {
			.numerator = 10000,
			.denominator = 250000,
		},
...
static long os04a10_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case SNS_V4L2_SET_HDR_ON:
	{
		...
		if (hdr_on) {
			memcpy(os04a10->cur_mode, &supported_modes[1], sizeof(struct os04a10_mode));
		}
		else {
