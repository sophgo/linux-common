...
/* Chip ID */
#define SC500AI_CHIP_ID_ADDR_H	0x3107
#define SC500AI_CHIP_ID_ADDR_L	0x3108
...
static int sc500ai_probe_index;
static const unsigned short sc500ai_i2c_list[] = {0x30};
static const int sc500ai_bus_map[MAX_SENSOR_DEVICE] = {1, -1, -1, -1, -1, -1};

struct sc500ai_reg_list {
	u32 num_of_regs;
	const struct sc500ai_reg *regs;
};
...
struct sc500ai_mode {
	...
	sns_sync_info_t sc500ai_sync_info;
	struct sc500ai_reg_list reg_list;
};

/* Mode configs */
static struct sc500ai_mode supported_modes[] = {
	{
		.max_width  = 2880,
		.max_height = 1620,
		.width = 2880,
		...
		.sns_type = V4L2_SMS_SC500AI_MIPI_5M_60FPS_10BIT,
		.sns_type_name = "V4L2_SMS_SC500AI_MIPI_5M_60FPS_10BIT",
		.max_fps = {
			.numerator = 10000,
			.denominator = 600000,
		},
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_linear_1620p60_regs),
			.regs = mode_linear_1620p60_regs,
		},
	},
	{
		.max_width  = 2880,
		...
		.vts_def = 1500,
		.mipi_wdr_mode = MIPI_WDR_MODE_NONE,
		.sns_type = V4L2_SMS_SC500AI_2L_MIPI_5M_30FPS_10BIT,
		.sns_type_name = "V4L2_SMS_SC500AI_2L_MIPI_5M_30FPS_10BIT",
		.max_fps = {
			.numerator = 10000,
			.denominator = 300000,
		},
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_linear_1620p30_2l_regs),
			.regs = mode_linear_1620p30_2l_regs,
		},
	},
};
...
#define to_sc500ai(_sd)	container_of(_sd, struct sc500ai, sd)

/* Read registers up to 4 at a time */
static int sc500ai_read_reg(struct sc500ai *sc500ai, u16 reg, u32 len,
			    u32 *val)
{
...
static int sc500ai_write_reg(struct sc500ai *sc500ai, u16 reg, u32 len,
			     u32 __val)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc500ai->sd);
	int buf_i, val_i;
...
static int sc500ai_write_regs(struct sc500ai *sc500ai,
			      const struct sc500ai_reg *regs, u32 len)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc500ai->sd);
	int ret;
	u32 i;
	...
	for (i = 0; i < len; i++) {
		ret = sc500ai_write_reg(sc500ai, regs[i].address, 1,
					regs[i].val);
		if (ret) {
...
static int sc500ai_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	/* Initialize try_fmt */
	try_fmt->width = sc500ai->cur_mode->width;
	try_fmt->height = sc500ai->cur_mode->height;
			      ...
			      struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	fie->width  = sc500ai->cur_mode->width;
	fie->height = sc500ai->cur_mode->height;

	fie->interval.numerator   = sc500ai->cur_mode->max_fps.numerator;
	fie->interval.denominator = sc500ai->cur_mode->max_fps.denominator;
			   ...
			   struct v4l2_subdev_frame_size_enum *fse)
{
	...
	fse->min_width = sc500ai->cur_mode->width;
	fse->max_width = sc500ai->cur_mode->max_width;
	fse->min_height = sc500ai->cur_mode->height;
	fse->max_height = sc500ai->cur_mode->max_height;

	return 0;
}

static void update_pad_format(const struct sc500ai_mode *mode, struct v4l2_subdev_format *fmt)
{
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		update_pad_format(sc500ai->cur_mode, fmt);
	}
	mutex_unlock(&sc500ai->mutex);
...
static void sc500ai_standby(struct sc500ai *sc500ai)
{
	sc500ai_write_reg(sc500ai, 0x0100, REG_VALUE_08BIT, 0x00);
}

/* Start streaming */
static int start_streaming(struct sc500ai *sc500ai)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc500ai->sd);
	const struct sc500ai_reg_list *reg_list;
	const sns_sync_info_t *sync_info;
	int ret;

	reg_list = &sc500ai->cur_mode->reg_list;

	ret = sc500ai_write_regs(sc500ai, reg_list->regs, reg_list->num_of_regs);
	if (ret) {
		dev_err(&client->dev, "%s failed to set mode\n", __func__);
		return ret;
	}

	sync_info = &sc500ai->cur_mode->sc500ai_sync_info;

	if (sync_info->num_of_regs > 0) {
		ret = sc500ai_write_regs(sc500ai, (struct sc500ai_reg *)sync_info->regs,
	...
	ret =  __v4l2_ctrl_handler_setup(sc500ai->sd.ctrl_handler);
	if (ret)
		return ret;

	dev_info(&client->dev, "wdr_mode(%d) reg setting done\n", sc500ai->cur_mode->mipi_wdr_mode);

	return ret;
}

/* Stop streaming */
static int stop_streaming(struct sc500ai *sc500ai)
{
	sc500ai_standby(sc500ai);
	return 0;
}
...
static int set_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		 ...
		 */
		ret = start_streaming(sc500ai);
		if (ret)
			goto err_rpm_put;
	} else {
		stop_streaming(sc500ai);
		pm_runtime_put(&client->dev);
	}

	sc500ai->streaming = enable;
	mutex_unlock(&sc500ai->mutex);
...
error:
	stop_streaming(sc500ai);
	sc500ai->streaming = false;
	return ret;
}

/* Verify chip ID */
static int sc500ai_identify_module(struct sc500ai *sc500ai)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc500ai->sd);
	int ret;
	int val1, val2;
	...
	ret = sc500ai_read_reg(sc500ai, SC500AI_CHIP_ID_ADDR_H,
	...
	ret = sc500ai_read_reg(sc500ai, SC500AI_CHIP_ID_ADDR_L,
			       REG_VALUE_08BIT, &val2);

	read_data = ((val1 & 0xFF) << 8) | (val2 & 0xFF);

	//if (read_data != SC500AI_CHIP_ID) {
	//	dev_err(&client->dev, "chip id(%x) mismatch, read(%x)\n",
	//		SC500AI_CHIP_ID, read_data);
	//	return -EIO;
	//}
...
static void sc500ai_mirror_flip(struct sc500ai *sc500ai, int orient)
{
	int Filp = 0;
	int Mirror = 0;
	...
	sc500ai_write_reg(sc500ai, 0x3221, REG_VALUE_08BIT, Filp << 5 | Mirror << 1);
}

static int sc500ai_update_link_menu(struct sc500ai *sc500ai)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc500ai->sd);
	struct v4l2_ctrl_handler *ctrl_hdlr;
	struct v4l2_ctrl *ctrl;
	int wdr_index = SNS_CFG_TYPE_WDR_MODE;
	int id = sc500ai->module_index;
	int ret;
	int i;

	sc500ai_link_cif_menu[id][wdr_index] = sc500ai->cur_mode->mipi_wdr_mode;

	dev_info(&client->dev, "update mipi_mode:%lld", sc500ai_link_cif_menu[id][wdr_index]);

	ctrl_hdlr = sc500ai->sd.ctrl_handler;
	...
	for (i = 0; i < SNS_CFG_TYPE_MAX; i++) {
		ctrl = v4l2_ctrl_new_int_menu(ctrl_hdlr, &sc500ai_ctrl_ops, V4L2_CID_LINK_FREQ,
					      sc500ai_link_cif_menu[id][i], 0,
					       (const s64 *)sc500ai_link_cif_menu[id]);
...
static int sc500ai_get_info_from_dts(struct sc500ai *sc500ai, int index_id)
{
	...
	for (i = 0; i < LANE_MAX_NUM; i++) {
		sc500ai_link_cif_menu[index_id][SNS_CFG_TYPE_DATA_LANE0 + i] =
			i >= num_lanes ? -1 : lane[i];
		sc500ai_link_cif_menu[index_id][SNS_CFG_TYPE_PN_SWAP0 + i] =
			i >= num_lanes_swap ? 0 : lane_swap[i];
	}
	sc500ai_link_cif_menu[index_id][SNS_CFG_TYPE_MIPI_DEV] = mipi_dev;
	sc500ai_link_cif_menu[index_id][SNS_CFG_TYPE_MCLK_NUM] = mclk_num;
	sc500ai_link_cif_menu[index_id][SNS_CFG_TYPE_WDR_MODE] = wdr_mode;
	sc500ai_link_cif_menu[index_id][SNS_CFG_TYPE_DPHY_SETTLE] = hs_settle;
	sc500ai_link_cif_menu[index_id][SNS_CFG_TYPE_DPHY_EN] = dphy_enable;
	sc500ai_link_cif_menu[index_id][SNS_CFG_TYPE_PHY_MODE] = cif_mode;
	//type_mode
	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		if (!strcmp(type_name, supported_modes[i].sns_type_name)) {
			sc500ai->cur_mode = devm_kzalloc(&client->dev,
						 sizeof(struct sc500ai_mode), GFP_KERNEL);
			memcpy(sc500ai->cur_mode, &supported_modes[i], sizeof(struct sc500ai_mode));
		}
	}

	sc500ai->power_gpio = devm_gpiod_get(&client->dev,
			"power", GPIOD_OUT_LOW);
	if (IS_ERR(sc500ai->power_gpio))
		dev_err(&client->dev, "failed to get power-gpios\n");
	else
		gpiod_set_value_cansleep(sc500ai->power_gpio, 1);

	sc500ai->reset_gpio = devm_gpiod_get(&client->dev,
...
static long sc500ai_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case SNS_V4L2_GET_TYPE:
	{
		...
		type = sc500ai->cur_mode->sns_type;
	...
	case SNS_V4L2_SET_MIRROR_FLIP:
	{
		...
		sc500ai_mirror_flip(sc500ai, orient);
		break;
	}
	...
	case SNS_V4L2_SET_HDR_ON:
	{
		...
		memcpy(sc500ai->cur_mode, &supported_modes[1], sizeof(struct sc500ai_mode));

		sc500ai_update_link_menu(sc500ai);
		break;
	}

	case SNS_V4L2_SET_SNS_SYNC_INFO:
	{
		memcpy(&sc500ai->cur_mode->sc500ai_sync_info, arg, sizeof(sns_sync_info_t));
		break;
	}
...
static const struct media_entity_operations sc500ai_subdev_entity_ops = {
...
static int sc500ai_init_controls(struct sc500ai *sc500ai, int index_id)
{
	struct i2c_client *client = v4l2_get_subdevdata(&sc500ai->sd);
	...
	ctrl_hdlr = &sc500ai->ctrl_handler;
	ret = v4l2_ctrl_handler_init(ctrl_hdlr, 10);
	if (ret) {
		dev_err(&client->dev, "%s ctrl handler init failed (%d)\n",
			__func__, ret);
	...
	for (i = 0; i < SNS_CFG_TYPE_MAX; i++) {
		if (i == SNS_CFG_TYPE_WDR_MODE)
			sc500ai->cur_mode->mipi_wdr_mode = sc500ai_link_cif_menu[index_id][i];

		ctrl = v4l2_ctrl_new_int_menu(ctrl_hdlr, &sc500ai_ctrl_ops, V4L2_CID_LINK_FREQ,
					      sc500ai_link_cif_menu[index_id][i], 0,
					       (const s64 *)sc500ai_link_cif_menu[index_id]);
	...
	ret = v4l2_ctrl_new_fwnode_properties(ctrl_hdlr, &sc500ai_ctrl_ops,
					      &props);
	if (ret)
		goto error;

	sc500ai->sd.ctrl_handler = ctrl_hdlr;
...
static void sc500ai_free_controls(struct sc500ai *sc500ai)
{
	v4l2_ctrl_handler_free(sc500ai->sd.ctrl_handler);
	mutex_destroy(&sc500ai->mutex);
}
...
static int sc500ai_probe(struct i2c_client *client)
{
	...
	int index_id = sc500ai_probe_index;
	int addr_num = sizeof(sc500ai_i2c_list) / sizeof(unsigned short);
	...
	sc500ai = devm_kzalloc(&client->dev, sizeof(*sc500ai), GFP_KERNEL);
	if (!sc500ai)
		return -ENOMEM;
	...
	if (!of_property_read_u32(client->dev.of_node, "reg-addr", &i2c_addr) &&
		...
		ret = sc500ai_identify_module(sc500ai);
		if (ret) {
	...
	for (i = 0; i < addr_num && use_defualt; i++) {
		if (force_bus[index_id] < 0)
			bus_id = sc500ai_bus_map[index_id];
		else
		...
		client->addr = sc500ai_i2c_list[i];
		client->adapter = i2c_get_adapter(bus_id);
		sc500ai->client = client;
		v4l2_i2c_subdev_init(sd, client, &sc500ai_subdev_ops);
	...
	sc500ai->module_index = index_id;

	sc500ai->cur_mode = devm_kzalloc(&client->dev,
					sizeof(struct sc500ai_mode), GFP_KERNEL);
	memcpy(sc500ai->cur_mode, &supported_modes[0], sizeof(struct sc500ai_mode));

	memset(&sc500ai->cur_mode->sc500ai_sync_info, 0, sizeof(sns_sync_info_t));

	mutex_init(&sc500ai->mutex);

	ret = sc500ai_init_controls(sc500ai, index_id);
	if (ret)
		return ret;
	...
	sd->entity.ops = &sc500ai_subdev_entity_ops;
...
error_media_entity:
	media_entity_cleanup(&sc500ai->sd.entity);
...
MODULE_DESCRIPTION("sc500ai sensor driver");
