...
static int os04a10_bus_map[MAX_SENSOR_DEVICE] = {5, 1, -1, -1, -1, -1};
...
static struct os04a10_mode supported_modes[] = {
	{
		...
		.sns_type = V4L2_OV_OS04A10_MASTER_MIPI_4M_1440P_2L_10BIT,
		.sns_type_name = "V4L2_OV_OS04A10_MASTER_MIPI_4M_1440P_2L_10BIT",
		.max_fps = {
			.numerator = 10000,
			.denominator = 300000,
		},
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_2560x1440_10bit_master_regs),
			.regs = mode_2560x1440_10bit_master_regs,
		},
	},
	{
		.max_width = 2560,
		...
		.vts_def = 2432,
		.mipi_wdr_mode = MIPI_WDR_MODE_NONE,
		.sns_type = V4L2_OV_OS04A10_SLAVE_MIPI_4M_1440P_2L_10BIT,
		.sns_type_name = "V4L2_OV_OS04A10_SLAVE_MIPI_4M_1440P_2L_10BIT",
		.max_fps = {
			.numerator = 10000,
			.denominator = 300000,
		},
		.reg_list = {
			.num_of_regs = ARRAY_SIZE(mode_2560x1440_10bit_slave_regs),
			.regs = mode_2560x1440_10bit_slave_regs,
		},
	},
};
			      ...
			      struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	if (os04a10->cur_mode->mipi_wdr_mode == MIPI_WDR_MODE_NONE) {
		fie->interval.numerator   = os04a10->cur_mode->max_fps.numerator;
		fie->interval.denominator = os04a10->cur_mode->max_fps.denominator;
	} else {
		fie->interval.numerator   = os04a10->cur_mode->wdr_max_fps.numerator;
		fie->interval.denominator = os04a10->cur_mode->wdr_max_fps.denominator;
	}
...
static int os04a10_get_info_from_dts(struct os04a10 *os04a10, int index_id) {
	...
	for (i = 0; i < LANE_MAX_NUM; i++) {
		os04a10_link_cif_menu[index_id][SNS_CFG_TYPE_DATA_LANE0 + i] =
			i >= num_lanes ? -1 : lane[i];
		os04a10_link_cif_menu[index_id][SNS_CFG_TYPE_PN_SWAP0 + i] =
			i >= num_lanes_swap ? 0 : lane_swap[i];
	}
	os04a10_link_cif_menu[index_id][SNS_CFG_TYPE_MIPI_DEV] = mipi_dev;
	os04a10_link_cif_menu[index_id][SNS_CFG_TYPE_MCLK_NUM] = mclk_num;
	os04a10_link_cif_menu[index_id][SNS_CFG_TYPE_WDR_MODE] = wdr_mode;
	os04a10_link_cif_menu[index_id][SNS_CFG_TYPE_DPHY_SETTLE] = hs_settle;
	os04a10_link_cif_menu[index_id][SNS_CFG_TYPE_DPHY_EN] = dphy_enable;
	os04a10_link_cif_menu[index_id][SNS_CFG_TYPE_PHY_MODE] = cif_mode;
	//type_mode
	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		if (!strcmp(type_name, supported_modes[i].sns_type_name)) {
			...
			memcpy(os04a10->cur_mode, &supported_modes[i], sizeof(struct os04a10_mode));
		}
	}

	os04a10->power_gpio = devm_gpiod_get(&client->dev,
			"power", GPIOD_OUT_LOW);
	if (IS_ERR(os04a10->power_gpio))
		dev_err(&client->dev, "failed to get power-gpios\n");
	else
		gpiod_set_value_cansleep(os04a10->power_gpio, 1);

	os04a10->reset_gpio = devm_gpiod_get(&client->dev,
...
static long os04a10_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case SNS_V4L2_GET_TYPE:
	{
		...
		type = os04a10->cur_mode->sns_type;
	...
	case SNS_V4L2_SET_HDR_ON:
	{
		...
		if (hdr_on)
			...
			memcpy(os04a10->cur_mode, &supported_modes[os04a10->module_index], sizeof(struct os04a10_mode));
...
static int os04a10_probe(struct i2c_client *client)
{
	...
	snprintf(sd->name, sizeof(sd->name), "cam%d_%s %s",
		 os04a10->module_index, "os04a10_sync", dev_name(sd->dev));
...
static void os04a10_remove(struct i2c_client *client)
{
	...
	pr_info("== os04a10_remove_index = %d ==\n", os04a10_probe_index);
...
MODULE_DESCRIPTION("os04a10_sync sensor driver");
