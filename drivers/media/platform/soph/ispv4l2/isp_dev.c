				 ...
				 struct v4l2_async_connection *asc)
{
	...
	// struct sop_isp_async_subdev *s_asc = container_of(asc,
	// 				struct sop_isp_async_subdev, asc);
	struct sop_isp_async_subdev *isp_async = &isp_dev->isp_async;

	if (isp_dev->num_sensors == ARRAY_SIZE(isp_dev->sensors))
		return -EBUSY;

	isp_dev->sensors[isp_dev->num_sensors].mbus = isp_async->mbus;
...
/*
	Not use the removed
	v4l2_async_notifier_parse_fwnode_endpoints() helper
	in linux_6.12.
*/
...
static int isp_parse_of(struct sop_isp_device *dev)
{
	...
	struct v4l2_async_connection *asc;
	struct sop_isp_async_subdev *isp_async = &dev->isp_async;
	int ret;

	ep = fwnode_graph_get_endpoint_by_id(dev_fwnode(dev->dev), 0, 0, 0);
	if (!ep) {
		v4l2_info(&dev->v4l2_dev, "No fwnode endpoint found!\n");
		return 0;
	}
	...
	if (ret) {
		v4l2_info(&dev->v4l2_dev, "Failed to parse %pOF\n", to_of_node(fwnode));
		ret = -EINVAL;
		goto out;
	}
	...
	if (vep.bus_type != V4L2_MBUS_BT656 &&
	    vep.bus_type != V4L2_MBUS_PARALLEL) {
		v4l2_info(&dev->v4l2_dev, "Unsupported bus type: %d\n", vep.bus_type);
		ret = 0;
		goto out;
	}

	isp_async->mbus.type = vep.bus_type;

	asc = v4l2_async_nf_add_fwnode(&dev->notifier, fwnode,
				       struct v4l2_async_connection);
	if (IS_ERR(asc)) {
		ret = PTR_ERR(asc);
		v4l2_info(&dev->v4l2_dev, "Failed to add fwnode to async notifier: %d\n", ret);
		goto out;
	}

	dev->isp_async.asc = asc;
out:
...
static void sop_isp_plat_remove(struct platform_device *pdev)
{
