...
struct legacy_timer_emu	usr_pic_timer;
atomic_t		dev_open_cnt;

spinlock_t event_lock;

spinlock_t raw_num_lock;
spinlock_t dq_lock[VI_MAX_CHN_NUM];
spinlock_t snr_node_lock[ISP_PRERAW_MAX];
...
struct _isp_dqbuf_q dqbuf_q[VI_MAX_CHN_NUM];
...
struct _isp_snr_cfg_queue isp_snr_i2c_queue[ISP_PRERAW_MAX];

struct _ai_isp_cfg_info ai_isp_cfg_info[ISP_PRERAW_MAX];
...
struct _membuf isp_bufpool[ISP_PRERAW_MAX] = {0};

struct mem_mapping vi_dma_buf_info;
struct mem_mapping yuv_dump_vb_info;
struct mem_mapping raw_dump_vb_info[2];

struct _mempool isp_mempool;
...
static void v4l2_sys_reg_write_mask(uintptr_t address, u32 mask, u32 data)
{
	u32 value;
...
{
    // 核心修正：第一个参数传物理地址addr_p（而非虚拟地址）
    arch_sync_dma_for_cpu((phys_addr_t)addr_p, len, DMA_FROM_DEVICE);
...
{
    // 核心修正：第一个参数传物理地址addr_p（而非虚拟地址）
    arch_sync_dma_for_device((phys_addr_t)addr_p, len, DMA_TO_DEVICE);
...
static int vi_get_pipe_frame(struct sop_vi_dev *videv, int vi_pipe,
...
static int vi_release_pipe_frame(struct sop_vi_dev *videv,
	const enum sop_isp_raw raw_num)
{
	u8 i = 0;
	int ret = 0;
...
static int vi_get_chn_frame(struct sop_vi_dev *videv,
...
static int vi_release_chn_frame(struct sop_vi_dev *videv)
{
...
static void sop_isp_rdy_buf_queue(struct sop_vi_dev *vdev, struct sop_isp_buf *b, const u8 chn_num)
{
	unsigned long flags;
...
static struct sop_isp_buf *sop_isp_rdy_buf_next(struct sop_vi_dev *vdev, const u8 chn_num)
{
	unsigned long flags;
...
static void sop_isp_rdy_buf_pop(struct sop_vi_dev *vdev, const u8 chn_num)
{
	unsigned long flags;
...
static void sop_isp_rdy_buf_remove(struct sop_vi_dev *vdev, const u8 raw_num)
{
	struct sop_isp_buf *b = NULL;
	int index;
...
static void _vi_pre_fe_get_dma_size(struct isp_ctx *ictx, u8 pipe)
{
	u32 bufsize = 0;
	u8  i = 0;
	u8  pre_fe_buf_num = OFFLINE_RAW_BUF_NUM;
...
/*
static int usr_pic_timer_init(struct sop_vi_dev *vdev)
{
#ifdef PORTING_TEST
	usr_trigger = true;
#endif
...
*/
static u32 _isp_event_to_v4l2_event(const u32 type)
{
	switch (type) {
...
static void _isp_v4l2_event_queue(struct video_device *vfd,
...
static void sop_isp_dqbuf_list(struct sop_vi_dev *vdev, const u32 frm_num, const u8 chn_id)
{
	unsigned long flags;
...
void _isp_yuv_bypass_trigger(struct sop_vi_dev *vdev, const enum sop_isp_raw raw_num, const u8 hw_chn_num)
{
	struct isp_ctx *ctx = &vdev->ctx;
...
static void _vi_pre_fe_ctrl_setup(struct sop_vi_dev *vdev, u8 chn, enum sop_isp_raw raw_num)
{
	struct isp_ctx *ictx = &vdev->ctx;
...
/*
static void _vi_bw_cal_set(struct sop_vi_dev *vdev)
{
	struct isp_ctx *ctx = &vdev->ctx;
...
static int vi_start_streaming(struct sop_vi_dev *vdev)
{
	enum sop_isp_raw raw_num = ISP_PRERAW0;
...
static int vi_stop_streaming(struct sop_vi_dev *vdev)
{
	struct _isp_dqbuf_n *n = NULL;
...
inline void _post_rgbmap_update(struct isp_ctx *ctx, const enum sop_isp_raw raw_num, const u32 frm_num)
{
...
void _splt_hw_enque(struct sop_vi_dev *vdev, const enum sop_isp_raw hw_raw_num)
{
	struct isp_ctx *ctx = &vdev->ctx;
	struct isp_buffer *b = NULL;
...
static void vi_destory_event_thread(struct sop_vi_dev *vdev, const enum sop_isp_raw raw_num)
{
	u8 th_id = raw_num;
...
static int vi_create_event_handler_thread(struct sop_vi_dev *vdev, const enum sop_isp_raw raw_num)
{
	struct sched_param param;
	char name[32];
	int th_id = raw_num;
...
void _vi_sw_init(struct sop_vi_dev *vdev)
{
	struct isp_ctx *ctx = &vdev->ctx;
	struct sop_vi_ctx *vi_proc_ctx = NULL;
	u8 i = 0, j = 0;
...
static int vi_mac_clk_ctrl(struct sop_vi_dev *vdev, u8 mac_num, u8 enable)
{
	int rc = 0;
...
#ifndef FPGA_PORTING
int _vi_clk_ctrl(struct sop_vi_dev *vdev, u8 enable)
{
	u8 i = 0;
	int rc = 0;
...
int get_dma_buf_size(struct sop_vi_dev *vdev)
{
	struct isp_ctx *ctx = &vdev->ctx;
	u32 tmp_size = 0;
	u32 buf_size = 0;
...
static void _vi_err_handler(struct sop_vi_dev *vdev, const enum sop_isp_raw err_raw_num)
{
	struct isp_ctx *ctx = &vdev->ctx;
	enum sop_isp_raw raw_num;
...
int _vi_err_handler_thread(void *arg)
{
	struct sop_vi_dev *vdev = (struct sop_vi_dev *)arg;
...
int _vi_event_handler_thread(void *arg)
{
	...
	while (1) {
		...
		ret = wait_event_timeout(vdev->vi_event_th[chn_num].wq,
...
int _vi_preraw_thread(void *arg)
{
	struct sop_vi_dev *vdev = (struct sop_vi_dev *)arg;
	enum sop_isp_raw raw_num = ISP_PRERAW0;
	struct isp_ctx *ctx = &vdev->ctx;
...
int _vi_vblank_handler_thread(void *arg)
{
	struct sop_vi_dev *vdev = (struct sop_vi_dev *)arg;
	enum sop_isp_raw raw_num = ISP_PRERAW0;
...
static void vi_irq_handler(struct sop_vi_dev *vdev)
{
	struct isp_ctx *ctx = &vdev->ctx;
...
static int sop_isp_g_input(struct file *file, void *priv, unsigned int *i)
{
...
static int sop_isp_s_input(struct file *file, void *priv, unsigned int i)
{
	struct sop_vi_dev *vdev = video_drvdata(file);
...
static int sop_isp_enum_input(struct file *file, void *priv, struct v4l2_input *inp)
{
	int rc = 0;
...
static int sop_isp_g_param(struct file *file, void *fh, struct v4l2_streamparm *a)
{
	int rc = 0;
...
static int sop_isp_s_param(struct file *file, void *fh, struct v4l2_streamparm *a)
{
	int rc = 0;
...
static int sop_isp_dqbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	struct sop_vi_dev *videv = video_drvdata(file);
	struct video_device *vdev = video_devdata(file);
	struct sop_isp_buf *b = NULL;
...
static int sop_isp_expbuf(struct file *file, void *priv, struct v4l2_exportbuffer *p)
{
	struct sop_vi_dev *videv = video_drvdata(file);
	struct video_device *vdev = video_devdata(file);
	struct sop_isp_buf *sop_vb, *tmp;
...
int vi_core_init(struct platform_device *pdev)
{
	...
	videv->ddr_retrain_reg = ioremap(VI_DDR_RETRAIN_REG, 0x4);
	if (!videv->ddr_retrain_reg) {
