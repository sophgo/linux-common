...
/*
static int ispblk_blc_config(struct isp_ctx *ctx, u32 blc_id, bool en, bool bypass)
{
	int id = blc_find_hwid(blc_id);
	uintptr_t blc;
...
static void _ispblk_ldci_cfg_update(struct isp_ctx *ctx, enum sop_isp_raw raw_num)
{
	uintptr_t ldci = ctx->phys_regs[ISP_BLK_ID_LDCI];
