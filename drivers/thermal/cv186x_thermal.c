...
static int cv186x_read_temp(struct thermal_zone_device *tz, int *temperature)
{
	struct cv186x_thermal_zone *ctz = thermal_zone_device_priv(tz);
...
static int cv186x_thermctl_get_trend(struct thermal_zone_device *tz,
				     ...
				     enum thermal_trend *trend)
{
	...
	if (temp > trip->temperature) {
		if (last_temp[zone->ch] != INT_MIN && temp >= last_temp[zone->ch])
			*trend = THERMAL_TREND_RAISING;
		else
			*trend = THERMAL_TREND_STABLE;
	} else if (temp < (trip->temperature - hyst)) {
		*trend = THERMAL_TREND_DROPPING;
	} else {
		*trend = THERMAL_TREND_STABLE;
	}

	last_temp[zone->ch] = temp;

	return 0;
}

static const struct thermal_zone_device_ops cv186x_thermal_ops = {
...
static void cv186x_thermal_remove(struct platform_device *pdev)
{
